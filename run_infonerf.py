...
def render(H, W, focal, chunk=1024*32, rays=None, c2w=None, ndc=False,
                  near=0., far=1.,
                  use_viewdirs=True, c2w_staticcam=None, depths=None,
                  **kwargs):
    """Render rays
    Args:
...
def create_nerf(args):
    ...
    """
    embed_fn, input_ch = get_embedder(args.multires, args.i_embed) # 10

    input_ch_views = 0
    embeddirs_fn = None
    if args.use_viewdirs:
        embeddirs_fn, input_ch_views = get_embedder(args.multires_views, args.i_embed) # 4
    output_ch = 5 if args.N_importance > 0 else 4
    skips = [4]
    if args.alpha_model_path is None: # default
        # D = 8, W = 256
...
def render_rays(ray_batch,
    ...
    if perturb > 0.:
        ...
        # if pytest: # Default: false
        #     np.random.seed(0)
        #     t_rand = np.random.rand(*list(z_vals.shape))
        #     t_rand = jt.float32(t_rand)
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        if args.N_rand is not None: # Default
            rays_o, rays_d = get_rays(H, W, focal, jt.float32(rgb_pose))  # (H, W, 3), (H, W, 3)
        ...
        if args.entropy and (args.N_entropy !=0):
            ...
            rays_o, rays_d = get_rays(H, W, focal, jt.float32(pose))  # (H, W, 3), (H, W, 3)
